...
/**
 * admin_adddocument method
 * @param string $user_id
 * 
 */
	public function admin_adddocument($user_id=0){
		$this->layout="admindefault";
		$this->adminsessionchecked();
		if($this->request->is('post')){
		...
		if($user_id>0){
			$finduser['User.id']=$user_id;
		}
		$users = $this->User->find('list',array('conditions'=>$finduser));
		//doc type section
		$this->loadModel('DocumentType');
		$fidcond = array(
			'DocumentType.is_user_provide'=>'0',
			'DocumentType.is_blocked'=>'0',
			'DocumentType.is_deleted'=>'0'
		);
		$this->DocumentType->displayField='name';
		$documentTypes = $this->DocumentType->find('list',array('conditions'=>$fidcond));
		$documentTypes['0']='Select One';
		ksort($documentTypes);
		$this->set(compact(array('users','documentTypes')));
